...
export const SignInPhone = ({ route, navigation }) => {
    ...
    const phoneRegex = /^\+?\d{10}$/;
        if (!phoneRegex.test(userPhone)) {
            ...
            setErrorNumero('Un code va vous être envoyer par sms afin, de vous idéntifier');
        }
  };
    ...
    <View style={Styles.container}>
        <ImageBackground style={Styles.bgGradient} source={require('../../assets/images/Background.png')}>
           <View style={[Styles.ViewText, Styles.mt50]}>
                <Text style={[Styles.textTitleWhite2, {top:100, left:50}]}>S'INSCRIRE</Text>
            </View>
            <SafeAreaView style={[Styles.ViewInput, {top: 180}]}>
                <TextInput
                    ...
                    value={userPhone}
                />
                {userPhone ? <Text style={[Styles.textError, { top: 10, }]}>{errorNumero}</Text> : <Text style={[Styles.textWhite4, { top: 10, width: '70%', }]}></Text>}
            ...
            <View style={[Styles.ViewBtn2, {top: 250}]}>
                <TouchableOpacity
                    style={Styles.btn}
                    onPress={() => navigation.navigate('Confirmation numero', {userConsent : consentement, routeName : routeChoice, loveCoach : loveCoach, userPhone: userPhone})}
