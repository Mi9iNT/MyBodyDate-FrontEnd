...
export const Compte = ({ route, navigation }) => {
  ...
  const validateEmail = (inputEmail) => {
    ...
    setErrorEmail(emailRegex.test(inputEmail) ? '' : 'Veuillez entrer une adresse e-mail valide.');
  };

  const validatePhone = (numero) => {
    const phoneRegex = /^\+?\d{10}$/;
    setErrorNumero(phoneRegex.test(numero) ? '' : 'Numéro de téléphone invalide. Veuillez respecter le format "+33 0 00 00 00"');
  };

  const validatePassword = (inputPassword) => {
    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@#$%^&?!])[A-Za-z\d@#$%^&?!]{8,}$/;
    setErrorPassword(passwordRegex.test(inputPassword) ? '' : 'Le mot de passe doit contenir : au moins une lettre majuscule, au moins une lettre minuscule, au moins un chiffre, au moins un caractère spécial (@#$%^&?!), et au moins huit (8) caractères.');
  };

  const emailTxt = routeChoice === "S'inscrire" ? 'Allez dans votre boite de messagerie pour confirmer votre email' : '';
      ...
      <ImageBackground style={[Styles.bgGradient, { top: 0, height: 810 }]} source={require('../../assets/images/Background.png')}>
        ...
        {routeChoice === 'Connexion numero' ? (
          ...
          <SafeAreaView style={[Styles.ViewInputEmail, { top: 30 }]}>
            ...
            />
            {errorPassword ? (
              <Text style={[Styles.textError2, { top: 40 }]}>{errorPassword}</Text>
            ) : (
              <Text style={[Styles.textWhite4, { top: 40, width: '80%' }]}>Pour garantir la sécurité de votre{"\n"}mot de passe, il doit contenir{"\n"}:{"\n"}-au moins une lettre majuscule{"\n"}-au moins une lettre minuscule{"\n"}-au moins un chiffre{"\n"}-au moins un caractère spéciale{"\n"}(@#$%^&?!){"\n"}-au moins huit (8) caractères</Text>
            )}
          </SafeAreaView>
        )}
